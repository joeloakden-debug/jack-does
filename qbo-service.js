...
/**
 * Get tax codes (e.g. GST, GST/PST BC, HST ON, etc.)
 */
async function getTaxCodes(clientId = 'default') {
  const qb = await getQBClient(clientId);
  return qbPromise(qb, 'findTaxCodes', { fetchAll: true });
}
...
/**
 * Find a vendor by display name, or create one if not found.
 * @param {string} vendorName - The vendor's display name
 * @param {string} clientId
 * @returns {Object} { Id, DisplayName }
 */
async function findOrCreateVendor(vendorName, clientId = 'default') {
  const qb = await getQBClient(clientId);
  // Search for existing vendor
  const result = await qbPromise(qb, 'findVendors', {
    DisplayName: vendorName,
  });
  const vendors = result?.QueryResponse?.Vendor || [];
  const match = vendors.find(v => v.DisplayName.toLowerCase() === vendorName.toLowerCase());
  if (match) return { Id: match.Id, DisplayName: match.DisplayName };

  // Create new vendor
  const created = await qbPromise(qb, 'createVendor', { DisplayName: vendorName });
  return { Id: created.Id, DisplayName: created.DisplayName };
}

/**
 * Create a Purchase (Expense) transaction in QuickBooks.
 * PaymentType "Cash" with AccountRef = the paying account (e.g. Shareholder Loan).
 * @param {Object} purchase - { date, memo, accountId, accountName, lines, taxCodeId?, globalTaxCalc? }
 * @param {string} clientId
 */
async function createPurchase(purchase, clientId = 'default') {
  const qb = await getQBClient(clientId);
  ...
  const lines = purchase.lines.map(line => {
    ...
    // Apply tax code to each line if provided (e.g. "TAX" or "NON" in QBO)
    if (line.taxCodeId) {
      lineObj.AccountBasedExpenseLineDetail.TaxCodeRef = { value: String(line.taxCodeId) };
    }
    return lineObj;
  });
  ...
  const purchaseObj = {
    ...
    PrivateNote: purchase.memo || 'Auto-generated by jack does',
    Line: lines,
  };
